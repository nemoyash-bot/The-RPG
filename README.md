# ⚔️ The RPG

A Python RPG that I (an 11 yr old) am building from scratch!

## 🎮 About the Game

**The RPG** is a turn-based adventure game where you explore dangerous forests, fight enemies, collect gold, buy equipment, complete quests, and become stronger.

The game is being developed in Python.

## 📌 Current Version

### Version 1.3 — Dark Forest & Valoria

The current version introduces:

- 🌲 Multiple forest areas
- 🌑 Dark Forest
- 🏰 Valoria
- ⚔️ Turn-based combat
- 👾 Multiple enemy types
- 💰 Gold and rewards
- 🛡️ Weapons and armor
- 🧪 Potions
- ☠️ Status effects
- 📜 Quests
- 🧙 NPCs
- 💾 Save/load system
- 📈 Leveling and EXP

## 👾 Enemies

The game currently includes enemies such as:

- Shadow Specter
- Blighted Beast
- Corrupted Knight
- Corrupted Treant
- Venomous Stalker
- Corrupted Drake
- Bandit Chieftain
- Alpha Dire Wolf
- Hobgoblin
- Bear
- Wolf
- Bandit
- Goblin

Some enemies have special abilities like:

- 💨 Dodge
- 🩸 Life Steal
- ☠️ Poison
- 🛡️ Armor Break
- 💥 Critical Hits

## 🗺️ Roadmap

### ✅ Version 1.1.5
- New enemies
- Stronger story creatures
- Improved combat mechanics

### ✅ Version 1.2 — Deep Forest
- Deep Forest Edge
- Corrupted Treants
- Venomous Stalkers
- Corrupted Drakes
- Armor Break
- Poison damage

### ✅ Version 1.3 — Dark Forest / Valoria
- Dark Forest Ruins
- Valoria
- Shadow Specters
- Blighted Beasts
- Corrupted Knights
- Shops
- Bounties
- Retired Warden Clark
- New equipment

### 🔒 Version 1.4 — Death Forest
- Death Forest
- Graveyard
- Necromancers
- Bone Golems
- Death Crawlers
- Heart Ward Relic

### 🔒 Version 1.5 — Forest Heart
- Forest Heart
- Elite Forest Sentinels
- Apex Boss
- Beast of Corruption
- Multi-phase boss fight

### 🔒 Version 2.0 — Capital / Second Sea
- Kingdom Gates
- Capital Hub
- Adventurer Guilds
- Factions
- Class Specializations
- Grand Dungeon

## 🛠️ Made With

- 🐍 Python IDLE
- 💻 Visual Studio Code
- 🌐 GitHub
## Update Releases
I will update this project approximately every  month for changes like V1.3 to V1.4

whereas for large updates will take 3 months.

## 👨‍💻 Development

This project was built from scratch as a personal RPG project.

Development started in 2026, and the first major version was built in about three weeks.

More updates coming soon! 🚀

## 📜 License

This project is currently for learning and personal development.
